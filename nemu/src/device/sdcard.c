...
static void sdcard_handle_cmd(int cmd) {
    switch (cmd) {
        ...
        case MMC_SEND_OP_COND: base[SDRSP0] = 0x80ff8000;
            break;
        ...
        case MMC_SEND_CSD:
            ...
            break;
        case MMC_SEND_EXT_CSD: read_ext_csd = true;
            addr = 0;
            break;
        ...
        case MMC_SET_BLOCK_COUNT: blkcnt = base[SDARG] & 0xffff;
            break;
        case MMC_READ_MULTIPLE_BLOCK: prepare_rw(false);
            break;
        case MMC_WRITE_MULTIPLE_BLOCK: prepare_rw(true);
            break;
        case MMC_SEND_STATUS: base[SDRSP0] = 0x900;
...
static void sdcard_io_handler(uint32_t offset, int len, bool is_write) {
    int idx = offset / 4;
    switch (idx) {
        case SDCMD: sdcard_handle_cmd(base[SDCMD] & 0x3f);
            break;
        case SDARG:
        case SDRSP0:
        case SDRSP1:
        ...
        case SDDATA:
            if (read_ext_csd) {
                ...
                if (!write_cmd) { ret = fread(&base[SDDATA], 4, 1, fp); } else {
